...
extension PushToTalkGatedControl {
    ...
    /// One begin outstanding at most: a press while the last is unanswered only supersedes a release (the begin is then
    /// pressed) — a second request would be refused as in progress, indistinguishably from the live transmission's.
