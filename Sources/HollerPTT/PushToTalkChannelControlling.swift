...
public protocol PushToTalkChannelControlling: Sendable {
    /// System callbacks, in the order the service delivered them. Single-consumer and created once: the gate owns
    /// the only subscription; a second `for await` would steal events from it.
