...
struct PushToTalkGatedControlQuickTapTests {
    ...
    @Test("a release that overtakes the suspended begin command (stop refused) is remembered: the begin is stopped")
    func releaseDuringSuspendedBeginCommand() async throws {
        let harness = try await makeGate()
        let (gate, service, inner) = (harness.gate, harness.service, harness.inner)
        await service.setHoldBegins(true)
        await service.setStopFailures(1)
        let press = Task { await gate.press() }  // the begin command suspends inside the service
        try #require(await eventually { await service.pendingBegins == 1 })
        await gate.release()  // the stop cannot be issued; the begin is already requested
        await service.releaseBegins()
        await press.value
        try await emitAndAwaitHandled(.beginTransmittingRequested(channel), on: service, gate: gate)
        #expect(await inner.recorded.contains("press") == false)  // the finger is up: never pressed
        #expect(await eventually { await service.count(.stop(channel)) == 2 })  // the system is asked to stop
        withExtendedLifetime(gate) {}
    }

    @Test("a press during a live system transmission (begun by the system UI) issues no begin request")
    func pressDuringSystemTransmissionIssuesNoBegin() async throws {
        let harness = try await makeGate()
        let (gate, service, inner) = (harness.gate, harness.service, harness.inner)
        try await emitAndAwaitHandled(.beginTransmittingRequested(channel), on: service, gate: gate)  // system button
        #expect(await inner.recorded.last == "press")
        await gate.press()  // the app button too
        #expect(await service.count(.begin(channel)) == 0)  // a duplicate would be refused, indistinguishably
        withExtendedLifetime(gate) {}
    }
