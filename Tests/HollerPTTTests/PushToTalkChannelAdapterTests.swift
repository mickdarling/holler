...
struct PushToTalkChannelAdapterTests {
    ...
    func commandErrors() async {
        ...
        await #expect(throws: PushToTalkServiceError.notPrepared) {
            try await adapter.join(Channel(id: kitchen, name: "K"))
        }
        await #expect(throws: PushToTalkServiceError.notPrepared) { try await adapter.leave(kitchen) }
        await #expect(throws: PushToTalkServiceError.notPrepared) {
            try await adapter.requestBeginTransmitting(kitchen)
        }
        await #expect(throws: PushToTalkServiceError.notPrepared) { try await adapter.stopTransmitting(kitchen) }
        await #expect(throws: PushToTalkServiceError.notPrepared) {
            try await adapter.setActiveSpeaker(nil, on: kitchen)
        }
    }
}
#endif
